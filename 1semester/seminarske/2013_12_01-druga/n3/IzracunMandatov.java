public class IzracunMandatov {
    ...
    public void izpisiTabelo() {
        ...
        for (int i = 1; i<=this.K; i++) {
            for (int j = 0; j<this.imenaStrank.length-i; j++) {
                int vrednostMandatov = 0;
                int[] koalicijskeStranke = new int[this.imenaStrank.length];
                int stevec = 0;
                for (int k = j; k <= i; k++, stevec++) {
                    vrednostMandatov += (int)tabela[stevec][1][1];
                    koalicijskeStranke[k] += k+1;
                } 
                if (vrednostMandatov >= this.M) {
                    System.out.print("Koalicija");
                    for (int k = 0; k<koalicijskeStranke.length; k++) {
                        if (koalicijskeStranke[k] != 0) {
                        System.out.printf(" %s", imenaStrank[koalicijskeStranke[k]-1]);
                        }
                    } 
                    System.out.printf(": %d %s.\n", vrednostMandatov, "mandatov");
