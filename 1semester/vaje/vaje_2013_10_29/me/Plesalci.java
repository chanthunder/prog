...
public class Plesalci {
    ...
    public static void main(String[] args) {
        ...
        System.out.println();
        int stevec = 1;
        for (int i=ustrezniGCD;i<=stMoskih;i+=ustrezniGCD) {
            for (int j=ustrezniGCD; j<=stZensk; j+=ustrezniGCD) {
                if (gcd(i, j) == ustrezniGCD) {
                    System.out.println("Par " + stevec + ": " + "moski " + i + "in zenska " + j);
                    stevec += 1;
                }
            }
        } 
