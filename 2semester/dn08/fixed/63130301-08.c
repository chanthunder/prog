...
void lcs(struct lines a, struct lines b, int ** c) {
    int i, j;
    for (i = 1; i < a.len + 1; ++i)
    {
        for (j = 1; j < b.len + 1; ++j)
        {
            if (a.lines[i - 1].len == b.lines[j - 1].len && memcmp(a.lines[i - 1].addr, b.lines[j - 1].addr, a.lines[i - 1].len) == 0)
            {
                c[i][j] = c[i-1][j-1] + 1;
            }
            else
...
/* void allLcs(struct int_array a, struct int_array b, int ** c, int i, int j) */
/* { */
/*     if (i == 0 || j == 0) */
/*     { */
/*         return; */
...
void diff3(char * input1_name, char * input2_name, char * out_name) {
    ...
    else
    {
        ...
        /* for (i = 0; i < input1.len; ++i) */
        /* { */
        /*     for (j = 0; j < input1.len; ++j) */
        /*     { */
        /*         if (i != j) */
        /*         { */
        /*             if (input1_hash.addr[i] == input1_hash.addr[j] && memcmp(input1.lines[i].addr, input1.lines[j].addr, input1.lines[i].len) != 0) */
        /*                 printf("Omg noob napaka. %dta in %dta vrstica imata enak hash\n", i, j); */
        /*         } */
        /*     } */
        /* } */
        /* for (i = 0; i < input2.len; ++i) */
        ...
        /*         if (i != j) */
        /*         { */
        /*             if (input2_hash.addr[i] == input2_hash.addr[j] && memcmp(input2.lines[i].addr, input2.lines[j].addr, input2.lines[i].len) != 0) */
        /*                 printf("Omg noob napaka. %dta in %dta vrstica imata enak hash\n", i, j); */
        /*         } */
        /*     } */
        /* } */
        lcs(input1, input2, matrika);
